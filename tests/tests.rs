...
mod tests {
    ...
    fn mnist() {
        ...
        let labels:ndarray::Array2<usize> = ndarray::Array::from_shape_vec((10000, 1), usize_labels).expect("Bad labels");
        let old_labels_shape = labels.shape();

        let usize_data:Vec<usize> = training_data.into_iter().map(|d|d as usize).collect();
        let data:ndarray::Array2<usize> = ndarray::Array::from_shape_vec((10000, 28*28), usize_data).expect("Bad data");
        ...
        // Reads metadata
        let metadata = std::fs::metadata("dense_mnist").expect("Couldn't get meta data on dense file.");
        // Checks size
